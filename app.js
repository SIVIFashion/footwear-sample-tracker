...
// ========================================
 async function exportToExcel() {
    ...
    alert('You have ' + samples.length + ' samples. Checking photos...');
    
    let withPhotos = 0;
    let totalPhotoSize = 0;
    
    for (let i = 0; i < samples.length; i++) {
        if (samples[i].photos && samples[i].photos.length > 0) {
            withPhotos++;
            if (samples[i].photos[0]) {
                totalPhotoSize += samples[i].photos[0].length;
            }
        }
    }
